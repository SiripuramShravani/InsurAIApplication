...
import Ivan from '../assets/InsurAI_Agent.png';
 
const AccessDenied = ({ isAuthError = false }) => {
  const navigate = useNavigate();
  const location = useLocation();
 
  const handleGoBack = () => {
    if (isAuthError) {
      // If it's an auth error, redirect to signin with return path
      navigate('/signin', {
        state: { from: location.state?.attemptedPath || '/dashboard' },
        replace: true
      });
    } else {
      // For regular 404s, just go home
      navigate('/');
    }
  };
  ...
  return (
    ...
    >
      ...
      >
        ...
        {isAuthError ? 'Sign In' : 'Go Back Home'}
      </Button>
